...
class Settings:
    ...
    BASE_DIR = r"C:\Users\Hesam\Desktop\Modular"
    DIR_GOLIVE = os.path.join(BASE_DIR, "GOLIVE")

    # OLD cluster artifacts
    AE_ZIP_PATH = os.path.join(BASE_DIR, "AE", "AE_artifacts.zip")
    KMEANS_CENTERS_CSV = os.path.join(
        BASE_DIR, "Kmeans", "Output", "kmeans_gmm_full_v3_centers.csv"
    )

    # Signal logs
    SIGNAL_LATEST = os.path.join(DIR_GOLIVE, "signal_latest.json")
    SIGNAL_HISTORY = os.path.join(DIR_GOLIVE, "signal_history.jsonl")
    DESKTOP_COPY = os.path.expanduser(r"~\\Desktop\\signal_latest.json")
    ...
    GATE_TF = "M5"  # فقط روی بسته‌شدن کندل جدید M5
    ...
    # Hedge:
    #   - ALLOW_HEDGE=False → پوزیشن خلاف جهت ممنوع
    #   - هم‌جهت مجاز طبق منطق Level/Story
    ALLOW_HEDGE = False
...
def save_trade_event(event: Dict[str, Any]):
    """
    ژورنال عمیق ترید:
      - هر تلاش order_send (REAL / FAIL / PAPER)
      - SKIPهای مهم با reason
      - DEALهای history_deals (PnL واقعی)
    """
    try:
...
def get_mt5_status(symbol: str) -> Dict[str, Any]:
    """
    Snapshot کامل از مارکت / اکانت / پوزیشن‌ها
    ضمیمه هر سیگنال در signal_history.
    """
...
def _compute_incremental_risk_pct(symbol: str,
    ...
    if same_symbol and same_cluster and same_state and same_dir:
        ...
        if level > prev_level:
            ...
            return 0.0, True  # بلاک incremental

    return base_risk, False  # داستان جدید → full risk
...
def _load_last_deal_ts() -> datetime:
    try:
        ...
        pass
    # اگر قبلاً چیزی نداشتیم → ۷ روز قبل
...
def log_new_deals():
    """
    هر بار حلقه:
      - history_deals_get از آخرین زمان لاگ شده
      - فقط MAGIC استراتژی
      - ثبت در trade_events.jsonl با event="DEAL"
    """
    ...
    try:
        while True:
            try:
                ...
                # 3) گیت روی بسته‌شدن کندل جدید M5
                m5_df = get_rates(
                    Settings.SYMBOL,
